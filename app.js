...
const STORAGE_KEY = 'autovalorpt-assistente-v2';
...
function restore() {
  try {
    ...
    state.pendingIntent = saved.pendingIntent || '';
...
function imageBlock(item, className) {
  ...
  if (item?.image) {
    ...
    img.loading = 'lazy';
    img.onerror = () => {
      wrap.innerHTML = '<div class="car-fallback">AV</div>';
    };
...
function whatsappText() {
  ...
  if (state.pendingIntent) lines.push(`Assunto: ${INTENTS[state.pendingIntent]?.short || state.pendingIntent}`);
...
function renderPurposeActions() {
  ...
  wrap.appendChild(el('div', 'action-heading', 'Escolha uma opção para continuar'));
  ...
  for (const [intent, icon, label] of actions) {
    ...
    button.type = 'button';
    button.innerHTML = `<span class="quick-icon">${icon}</span><span>${label}</span>`;
    button.addEventListener('click', () => startPurpose(intent));
...
function renderFollowupActions() {
  ...
  const other = el('button', 'followup-secondary', 'Adicionar outro assunto');
  other.type = 'button';
...
function selectVehicle(item) {
  ...
  addBubble('Boa escolha. Agora selecione apenas o assunto que pretende tratar primeiro.', 'bot');
...
async function sendMessage(message) {
  ...
  try {
    ...
    setComposer('Pode responder aqui ou adicionar outro assunto…');
  } catch {
    hideTyping();
...
if (restored) {
  ...
  if (state.vehicle) {
    if (state.pendingIntent) renderFollowupActions();
    else renderPurposeActions();
  } else {
    loadStock();
  }
}
