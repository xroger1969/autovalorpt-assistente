...
async function copyImageToBlob(sourceId, item, position) {
  ...
  return { type: 'IMAGE', url: blob.url, pathname: blob.pathname };
}
...
export async function listDrafts() {
  ...
  for (const blob of result?.blobs || []) {
    ...
    if (!previous || Number(match[2]) > previous.version) {
      latest.set(match[1], { version: Number(match[2]), url: blob.url });
    }
  }

  const items = [];
  for (const entry of latest.values()) {
    const draft = await readBlobJson(entry.url);
...
export async function getDraft(sourceId) {
  ...
  const newest = (result?.blobs || [])
    .map((blob) => {
      ...
      return match ? { version: Number(match[1]), url: blob.url } : null;
    })
    .filter(Boolean)
    .sort((a, b) => b.version - a.version)[0];
  return newest ? readBlobJson(newest.url) : null;
}
...
export async function publishDraft(draft) {
  if (!metaConfigured()) throw Object.assign(new Error('Ligação à Meta ainda não configurada.'), { code: 'meta_not_configured' });
  const media = Array.isArray(draft?.media) ? draft.media.filter((item) => item?.type === 'IMAGE' && item?.url).slice(0, MAX_MEDIA_ITEMS) : [];
  if (!media.length) throw Object.assign(new Error('Este rascunho ainda não tem fotografias publicáveis.'), { code: 'no_media' });

  let creationId;
  if (media.length === 1) {
    const created = await createContainer({ image_url: media[0].url, caption: String(draft.caption || '') });
    creationId = created.id;
  } else {
    const children = [];
    for (const item of media) {
      const child = await createContainer({ image_url: item.url, is_carousel_item: 'true' });
